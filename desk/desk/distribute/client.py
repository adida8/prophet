"""HTTPS client for the MTA webhook.

Sum-typed `DeliverResult` so the worker doesn't need to interpret
status codes — it just dispatches on `Ok | Retryable | Permanent`.
The decision table is in this module, in one place.

Status mapping (per the operator's contract):
...
from desk.distribute.signing import sign
...
# Treat these as transient. Everything else 4xx is permanent (caller
# bug, won't go away on retry).
...
class MTAClient:
    ...
    One client instance per worker process. The httpx client is created
    lazily on first deliver so tests can construct an MTAClient without
    a network stack.
    """
    ...
    async def deliver(self, body: bytes, *, now: int | None = None) -> DeliverResult:
        ...
        try:
            ...
            # Only close the lazily-created client; the injected one
            # belongs to the caller (typically tests).
        ...
        # Unknown status — be conservative and alert. The receiver
        # contract is well-defined; anything outside the known set is
        # likely a deployment or routing bug we want to see.
