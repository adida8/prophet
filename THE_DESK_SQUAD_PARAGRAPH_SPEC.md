...
- ✅ **PR Q2 — fold cards into `TeamNews`** (commit `ac224bc`). `CardStatus` dataclass + `cards: tuple[CardStatus, ...]` field on `TeamNews`. Builder consumes the new `card_rows` kwarg, drops `at_risk=0` rows, reconciles at-risk against `absences` (a player both at-risk AND already suspended is dropped from cards — substring fallback covers "Bellingham" vs "Jude Bellingham"). At-risk cards bump `materiality` from `none → low` only — never higher. `APIFootballRuntime.cards_for_iso3(iso3, competition)` is the hot-path read. `FootballSport.decide_and_explain` threads card_rows onto `build_team_news` keyed on `fx.competition_code`, gated on `DESK_CARD_AT_RISK` (default `1`). Hot path is byte-identical when the cache is cold (no rows → empty `cards` → `materiality` math unchanged).
- ✅ **PR Q3 — match-blurb dedicated-paragraph mandate** (commit `5335420`). Haiku system prompt rewritten with a `SQUAD PARAGRAPH` section mandating ONE dedicated paragraph covering opening XI + out + at-risk in priority order. Cards rendered in `_format_team_news` alongside absences + lineup. Word-count ceiling bumps `220 → 240` when either side has squad content (confirmed lineup OR absence OR at-risk card). `post_check` gains: (a) availability-keyword set extended with "at risk" / "one booking" / "one yellow" / "yellow accumulation" so a hallucinated card story is caught under `materiality=none`; (b) at-risk-as-fact guard — any at-risk player named within 6 words of "suspended" / "banned" / "missing" / etc. triggers a soft warning (hard fail under `DESK_TEAM_NEWS_BLURB_REQUIRED=1`).
- ✅ **PR Q4 — outright per-team `squad_note`** (commit `3cc9f6d`). `desk/outrights/squad_note.py` builds a deterministic per-team string from `TeamNews` — `Out: Name (injury), Name (suspension). One booking from a ban: Name.`, omitting empty clauses, empty string when nothing known. Reused `build_team_news` keyed on each ladder team's ISO3 (no signals, no lineup — outright surface is about between-match availability). Threaded into `publish.py:_ladder_dict` as a new `squad_note` field on every ladder row. **No ADR raised.** The outright JSON has no Pydantic schema, no `CONTRACT_CHANGELOG` entry, and no documented external subscribers — `CONTRACT_CHANGELOG.md` governs `MatchOutput` only. Adding a string field to a dict-shape ladder row is additive-optional. NOT a Haiku call — 48 teams × an LLM call per refresh would be both expensive and pointless for a one-line mechanical summary. Renderer can read the field unconditionally; cold-cache deploys emit empty strings for every team.
- ✅ **PR Q5 — `desk fetch-cards` CLI + daily-tick wiring** (commit `ac1a60e`). `desk/data/api_football/cards_refresh.py` orchestrator (mirror of `injuries_refresh.py` — same auth-abort discipline, same per-team failure isolation). New `desk fetch-cards [--db --season --competition --iso3]` CLI subcommand. Wired into `desk_refresh_loop.py` AFTER `fetch-injuries` (so the at-risk reconcile reads the latest suspension set), gated on `DESK_CARD_FETCH=1` + `API_FOOTBALL_KEY`. ~48 calls/tick (one per WC26 team) on the cheap `/players?team=&season=&league=` path — well inside the Pro cap (7,500/day). CLAUDE.md updated with the new env vars (`DESK_CARD_FETCH`, `DESK_CARD_AT_RISK`) and the daily-tick step ordering.
- ✅ **Live on staging + prod, 2026-05-30 evening.** `DESK_CARD_FETCH=1` + `DESK_INJURY_FETCH=1` + `DESK_LINEUP_FETCH=1` + `DESK_LINEUP_LOOP_ENABLED=1` + `DESK_BLURB_HAIKU=1` flipped on both Railway services. Side-fix: api-football `/teams?search=` rejects `&` and is prefix-sensitive — `"Bosnia and Herzegovina"` + `"DR Congo"` returned 0 hits, dragging fixture coverage to 91.7%. Registry strings updated to `"Bosnia"` + `"Congo DR"` (api-football canonical ids 1113 + 1508) in commit `ff79261`.

**Outstanding operator action:** WC26 yellow-card reset rule (`wipe_after_stage="quarter_final"` in `CARD_RULES["wc26"]`) was carried over from 2022's regulations and `DESK_CARD_FETCH=1` was flipped live before this was verified against the actual 2026 FIFA tournament regs. Post-QF at-risk flags will be wrong if 2026 diverges. Worth 15 minutes against the published 2026 regs before the tournament's group stage closes.

### Post-ship change — 2026-05-31: always-on squad paragraph

After operator eyeballing on staging revealed that 71 of 72 matches were correctly shipping with no squad paragraph (per the original "silence is the rule when there's no data" mandate — api-football carries virtually no pre-tournament injury data for national teams), the spec was changed to mandate a one-sentence **positive** squad paragraph in the no-data case. Reader expects to see SOMETHING about the squad on every match page; absolute silence reads as missing content.

Implementation:
- **Prompt change** — `SQUAD PARAGRAPH` section in `_SYSTEM_PROMPT` adds clause (4): when both sides have `materiality=none`, `lineup.state="unknown"`, and `cards=()`, write one short positive sentence. Examples: `"Both squads come through clean; no late absences flagged."` / `"Neither side carries any flagged availability concerns into kickoff."` / `"Squad picture is straightforward — nothing flagged either way."` Prompt is explicit: NEVER name a player in the no-data case, NEVER claim a specific injury / suspension / card status, vary the phrasing across matches to avoid repetition.
- **Post-check relaxation** — the `materiality=none` guard's broad keyword check (`_AVAILABILITY_KEYWORDS_RE`) replaced with a narrower negative-claim check (`_NEGATIVE_AVAILABILITY_CLAIM_RE`). Old guard rejected ANY mention of "injury" / "suspended" / "absent" / "at risk"; new guard rejects only phrasings that COMMIT to a specific negative state — `"X is injured"`, `"ruled out"`, `"out with a knock"`, `"one yellow from a ban"`, etc. Generic vocabulary (`"no flagged absences"`, `"at full strength"`, `"squad picture is clean"`) is now allowed. Hallucination protection comes primarily from the prompt's explicit positive-only instruction.
- **Caveat acknowledged:** api-football returning zero injury rows isn't the same as "the squad is healthy" — it could be no data at all. The operator accepted this tradeoff because (a) the alternative (silence on ~71 of 72 matches) was worse for editorial coverage, and (b) the prompt instructs Haiku to phrase the no-data sentence with deliberate hedging (`"as far as we know"` / `"nothing flagged either way"` / `"no late concerns surfaced"`).
