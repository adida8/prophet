...
A user — Adi, Faktor, an end-reader — must be able to pick up a Pick verdict and:
...
**The track-record page IS the credibility story.** Faktor doesn't have to take our word for the Brier number — they can read every prediction we've ever made.
...
Three other Desk specs already exist (`THE_DESK_SPEC.md`, `4_5`, `BACKTEST`). This one differs because it's not implementation-first — it's the **trust contract** between the engine and its consumers (Faktor, then end-readers). Every other spec answers "how do we build this?" This one answers "what makes it credible to ship?"
