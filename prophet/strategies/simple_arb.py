...
# ── Defaults (used when settings.json doesn't exist) ─────────────────
_DEFAULTS = {
    "yes_ceiling": 0.55,
    "no_floor": 0.50,
    "min_edge": 0.01,
...
def _load_settings() -> dict:
    ...
    try:
        ...
        return {
            "yes_ceiling": float(data.get("yes_ceiling", _DEFAULTS["yes_ceiling"])),
            "no_floor": float(data.get("no_floor", _DEFAULTS["no_floor"])),
            "min_edge": float(data.get("min_edge", _DEFAULTS["min_edge"])),
        }
...
class SimpleArbStrategy(Strategy):
    ...
    def evaluate(self, tick: dict) -> Signal | None:
        ...
        yes_ceiling = settings["yes_ceiling"]
