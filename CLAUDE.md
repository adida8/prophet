...
| `DESK_CARD_FETCH` | `0` | Squad-paragraph Q1/Q5. Set to `1` to run `desk fetch-cards` in the daily refresh tick — pulls api-football `/players?team=&season=&league=` per WC26 team (one call/team, ~48/tick) and derives `at_risk = (yellows == threshold-1)` from `CARD_RULES[fx.competition_code]`. Needs `API_FOOTBALL_KEY`. **Operator action pending before flipping live:** confirm the WC26 yellow-card reset rule (wipe-after-QF) against the published 2026 FIFA tournament regulations; the v1 rule follows 2022 guidance. Off by default. |
...
- **api-football team-search gotcha — `bih` + `cod` don't resolve out-of-the-box.** Their WC26-registry display names (`"Bosnia and Herzegovina"` and `"DR Congo"`) return zero hits from `/teams?search=`. api-football's canonical names are `"Bosnia & Herzegovina"` (id 1113) and `"Congo DR"` (id 1508). Don't send the `&` — the API rejects the call with `"Search field may only contain alpha-numeric characters and spaces"`. Working searches: `"Bosnia"` and `"Congo DR"`. Two teams uncovered drags WC26 fixture-coverage from 100% to 91.7% (below the 98% pre-publish WARN threshold in `desk/desk/data/api_football/coverage.py`). Fix is in the registry's search-term map.
