...
| `DESK_TEAM_NEWS_BLURB_REQUIRED` | `0` | When `1`, the Haiku blurb writer's team-news guards become hard fails — a `materiality=high` blurb that doesn't name an absent player falls back to the stub, ditto for confirmed-lineup blurbs that don't mention a starter or formation. Default `0` ships the guards as warnings only. |
