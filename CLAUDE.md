...
| `DESK_AUTORUN` | `1` | Emergency kill switch for the background refresh loop. Set to `0` to disable the loop entirely on a given Railway service (skips the boot tick too). Day-to-day enable/disable + frequency is controlled via the Ops dashboard's **Refresh control** panel at `/desk/ops` — that writes `{ops_root}/control.json`, which the loop polls every iteration. Leave this env var at default unless you need to pause without dashboard access. |
...
- **`desk_refresh_loop.py` runs on a cadence controlled by the Ops dashboard.** Default cadence on a fresh install: daily (1440 min). Operators toggle enabled/disabled + frequency at `/desk/ops` → **Refresh control**; changes land on the loop's next scheduling decision (no restart). State persists at `{ops_root}/control.json` so prod and staging each carry their own setting. `DESK_AUTORUN=0` is the deploy-time kill switch (use it only when the dashboard isn't reachable). Each tick runs `desk run --once` (timeout 600s) → `desk outrights` → `site/generate.py` → `desk fetch-signals` (if `DESK_SIGNALS_FETCH=1`) → `desk extract-signals` (if `DESK_SIGNALS_EXTRACT=1` + `ANTHROPIC_API_KEY` set). **Matches runs first** so the dashboard-critical artifacts always land before signals work can eat the time budget — signals enrich the *next* tick's matches (one-tick lag is fine; missing matches is not). Set `DESK_SIGNALS_EXTRACT_LIMIT=25` on prod to keep Haiku extraction inside its 600s timeout and cost predictable.
