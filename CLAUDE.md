...
| `DESK_LINEUP_LOOP_PUBLISH` | `0` | When `1`, the loop runs `desk run --once` + `site/generate.py` after every successful lineup fetch so blurbs reach the published surface immediately. **Default flipped to `0` on 2026-05-31** — the previous default of `1` was silently re-firing the full Haiku blurb writer across all 72 WC26 matches on every confirmed XI and burned $20 of Anthropic credit in a single day. With `0`, lineup data still lands in the api-football cache and the next scheduled daily tick picks it up. |
| `LEDGER_REFRESH_INTERVAL_SEC` | `1800` | Ledger wallet refresh cadence (was hardcoded `900`). Loop is **disabled by default** at `/desk/ops/schedules`; flip it on there if you need it back. |
...
| `DESK_API_BEARER_TOKEN` | unset | Token for the bearer-gated external GETs at `/api/desk/external/match/{match_id}` and `/api/desk/external/outright/{outright_id}` (single-item safety-net reads for MTA's "Refresh from desk" button). Both return the exact wire shape the push delivers — the outright route applies the same `outright_wire_payload` transform (content_type + top-level hard_signal_adjustments) the webhook uses. Shape `dtk_<32-byte URL-safe>`. **Without it both routes 404** — server.py omits the mount entirely so unconfigured deploys don't acknowledge the surface. |
...
- **`desk_lineups_refresh_loop.py` runs separately at a 15-minute cadence (`DESK_LINEUP_LOOP_TICK_SEC`), scoped to the next 2 hours of kickoffs (`DESK_LINEUP_LOOP_WINDOW_HOURS`).** Off by default; flip `DESK_LINEUP_LOOP_ENABLED=1` to start it AND turn it on at `/desk/ops/schedules`. Each tick calls `desk fetch-lineups --window-hours 2`; on any successful fetch (api-football flagged at least one fixture's confirmed XI) the loop also runs `desk run --once` + `site/generate.py` so the new starters reach the published JSON + static HTML before kickoff. **`DESK_LINEUP_LOOP_PUBLISH` default flipped to `0` on 2026-05-31** — auto-republishing across all 72 WC26 matches on every confirmed XI was silently re-firing the full Haiku blurb writer and burned $20 of Anthropic credit in a single day. Operator must explicitly set `DESK_LINEUP_LOOP_PUBLISH=1` to opt back in. Sits alongside the daily refresh loop in `main.py`; honours `DESK_AUTORUN=0` as the kill switch.
- **Unified background-loop control lives at `/desk/ops/schedules`** (spec: `SCHEDULES_ADMIN_SPEC.md`, registry: `loop_registry.py`). One row per loop with an on/off button — `desk_refresh`, `lineups_refresh`, `distribute`, `activity_aggregate`/`_seed`/`_prune`, `daily_report`, `site_qa`, `ledger_refresh`, `social_weekly`. State persists in `{ops_root}/schedules.json` (each loop polls `loop_registry.is_enabled(loop_id)` on every tick so flips land within ~60s without a restart). **`desk_refresh` is the one exception**: its enabled flag still lives in `control.json` so the Desk admin tab (which also owns the hour list) and the Schedules page stay in sync; the API delegates the desk_refresh row to control.json read/write. **The Schedules toggle is an *additional* gate**, not an override — env-var feature flags (`DESK_DISTRIBUTE_PUSH`, `DAILY_REPORT_ENABLED`, `DATABASE_URL`, etc.) still need to be set for the loop to actually do work. **Ledger refresh ships disabled by default** (was hardcoded 15-min; flipped off 2026-05-31). Re-enable at `/desk/ops/schedules` if needed.
- **Ops dashboard has four views** at `/desk/ops`: left-sidebar nav with **The Desk** (read-only run history with Date / Start / End / Trigger / Status / Pub. / Picks / Δ / Cost columns + **External providers** panel showing api-football + live Elo + openweathermap health + Sources read + News signals + per-run cost), **Schedules** (unified on/off for every background loop — see above), **Social** (social-queue approval), and **Desk admin** (master enable toggle for `desk_refresh` + a list of scheduled UTC hours with Add/Remove). The **External providers** panel calls `GET /api/desk/ops/data-sources` independently of the run-history fetch so a fresh-deploy with zero runs still surfaces provider health; each provider card shows a status pill (`live` / `fetch off` / `no cache yet` / `no key` / `probe-only`) + count of cached rows + recent fetch timestamps. **OpenWeatherMap intentionally surfaces as `probe-only`** — the key is verified but B.2 weather fetch/cache/hook isn't wired. All views share the same HTTP Basic auth gate (`DESK_OPS_USER` / `DESK_OPS_PASS`); `server.py` wraps the SPA catch-all under `/desk/ops/*` with that gate so `/desk/ops/admin` + `/desk/ops/schedules` can never be reached unauthed. The admin page writes `control.json` directly; the schedules page writes `schedules.json` (or `control.json` for the desk_refresh row).
